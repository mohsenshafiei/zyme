...
const formatDetails = (info: PackageInfo, stars: string | number) =>
  [
    transformIf(
...
const printTree = (root: string, branches: (string | number)[]) => {
  console.log();
  console.log(`${cnsl.yellowGreen("╭◉ ")}${cnsl.bold.white(root)}`);
  for (const [index, branch] of branches.entries()) {
    const prefix = index === branches.length - 1 ? "╰──‣" : "├──‣";
    console.log(cnsl.yellowGreen(prefix), cnsl.silver(branch as string));
  }
};
...
const getPackageJsonValues = async (): Promise<PackageJson | undefined> => {
